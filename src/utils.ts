...
export const parseZonesData  = (data: PanelData) =>  data.series[0].fields.reduce((res, prev, i) => {
      if(i == 0) {
  ...
  export const parseZonesRelativeWorkload = (data: PanelData) => {
      ...
      const zonesName = data.series[1].fields.find(el => el.name === "Zone")?.values.toArray()
    
      return zonesName ? zonesName.reduce((res, prev, i)=>{
           //@ts-ignore
           res[prev] = data.series[1].fields.find(el => el.name === "Relative workload")?.values.get(i)
